...
<main id="content">
    ...
    <div id="signUpWrap">
        ...
        <div id="signUpPopup">
            <div id="popupBox">
                <h3 class="screen_out">회원가입 동의</h3>
                <label id="allLabel"><input id="allCheckbox" type="checkbox" class="checkbox" >이용약관에 모두 동의합니다.</label>
                <label id="requiredLabel"><input id="requiredCheckbox" type="checkbox" class="checkbox" >개인정보 수집 및 이용에 대한 안내<span>(필수)</span></label>
                <p id="guide">제1장 총칙

                    제1조 (목적) 이 약관은 Lost Stations(이하 “회사”)가 온라인으로 제공하는 게임 및 이에 부수된 제반 서비스(이하 “게임서비스”)의 이용과 관련하여 회사와 회원 간의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
                    ...
                    제34조 (재판권 및 준거법) 본 약관은 대한민국 법률에 따라 규율되고 해석되며, 회사와 회원간에 발생한 분쟁으로 소송이 제기되는 경우, 법령에 정한 절차에 따른 법원을 관할 법원으로 합니다.~</p>
                <label id="choiceLabel"><input id="choiceCheckbox" type="checkbox" class="checkbox" >이벤트 등 프로모션 알림<span>(선택)</span></label>
                <div id="signUpMsg" class="msg">필수사항에 모두 동의해주세요.</div>
                <a class="popup btn" href="index.jsp">취소</a>
                <button id="agreeBtn" class="popup btn">동의</button>
            </div><!--//popupBox-->
        </div><!--//signUpPopup-->
        <div id="signUpBox">
            <form id="signUpForm" action="/user" method="post">
                <h3>아이디</h3>
                <input id="id" name="id" placeholder="이메일을 입력해 주세요.">
                <!-- <button id="idCheckBtn" class="btn normal" type="button">중복 확인</button> -->
                <div class="id msg">아이디를 바르게 입력해 주세요.</div><!--//id_msg-->
                <h3>비밀번호</h3>
                <input id="password" name="password" type="password" placeholder="숫자, 영어 조합 6~12자리">
                <div class="password msg">숫자, 영어 조합 6~12자리로 입력해 주세요.</div><!--//password msg-->
                <h3>비밀번호 재확인</h3>
                <input id="passwordCheck" name="passwordCheck" type="password">
                <div class="password_check msg">비밀번호가 일치하지 않습니다.</div><!--//password_check msg-->
                <h3>닉네임</h3>
                <input id="nickname" name="nickname" placeholder="숫자, 영어, 한글로 2~6자리">
                <!--<button id="nicknameCheckBtn" class="btn normal" type="button">중복 확인</button> -->
                <div class="nickname msg">이미 사용중인 닉네임입니다.</div><!--//nickname msg-->
                <h3>이름</h3>
...
<script>
    ...
    let $requiredCheckbox = $('#requiredCheckbox');
    let $choiceCheckbox = $('#choiceCheckbox');
    let $signUpMsg = $("#signUpMsg");
    let $agreeBtn = $("#agreeBtn");
    let $signUpForm = $("#signUpForm");

    let $id = $("#id");
    let $idMsg = $(".id");
    let $password = $("#password");
    let $passwordMsg = $(".password");
    let $passwordCheck = $("#passwordCheck");
    let $passwordCheckMsg = $(".password_check");
    let $nickname = $("#nickname");
    let $nicknameMsg = $(".nickname");
    let $name = $('#name');
    let $nameMsg = $(".name");

    let $idCheckBtn = $('#idCheckBtn');
    let $nicknameCheckBtn = $('#nicknameCheckBtn');

    let idExp = /^[0-9a-zA-Z]{5,30}@[a-z]{1,10}\.[a-z]{2,3}$/i;
    let passwordExp = /^.*(?=.{6,12})(?=.*[\0-9])(?=.*[a-zA-Z]).*$/;
    let nicknameExp = /^[0-9a-zA-Z가-힣]{2,6}$/;
    let nameExp = /^[가-힣]{2,6}$/;

    let idFlag = false;
    let nicknameFlag = false;
    
    //아이디 유효성 검사 및 중복확인 수행하라는 함수
    $idCheckBtn.on('click',testId);//$idCheckBtn.click end
    //아이디를 중간에 바꿨을 때 다시 검사해주라는 함수
    $idCheckBtn.on('blur',testId);//$idCheckBtn.click end
    
  	//닉네임 유효성 검사 및 중복확인 수행하라는 함수
    $nicknameCheckBtn.on('click', testNickname);//$nicknameCheckBtn.click end
    //닉네임을 중간에 바꿨을 때 다시 검사해주라는 함수
    $nicknameCheckBtn.on('blur', testNickname);//$nicknameCheckBtn.click end
    
    
    //아이디 유효성 검사(중복 확인까지) 함수
    function testId() {
        let id = $id.val();
        if(idExp.test(id)) {
        	 //let id = $id.val();
             $.ajax({
                 ...
                 success:function (data) {
                	 console.log(data)
                        if(data==0) {
                        	$idMsg.show().removeClass("no").addClass("ok").text("좋은 아이디네요!");
                        	idFlag = true;
                        	
                        }else {
                        	$idMsg.show().removeClass("ok").addClass('no').text("이미 존재하는 아이디입니다.");
                        	idFlag = false;
                        }
                 },//success end
            })//ajax end
            ...
            $idMsg.show().removeClass("ok").addClass('no').text("아이디를 바르게 입력해 주세요.");
            return false;
        }
    }//testId() end
    
  	//닉네임 유효성 검사(중복 확인까지) 함수
    function testNickname() {
    	let nickname = $nickname.val();
    	if(nicknameExp.test(nickname)){
	        $.ajax({
	            url:"/ajax/check/nickname",
	            dataType:"json",
	            type:"get",
	            ...
	            success:function (data) {
	           	 console.log(data);
	                   if(data>=1) {
	                	   $nicknameMsg.show().removeClass("ok").addClass("no").text("이미 존재하는 닉네임입니다.");
	                	   nicknameFlag = false;
	                   }else {
	                	   $nicknameMsg.show().removeClass("no").addClass("ok").text("좋은 닉네임이네요!");
	                	   nicknameFlag = true;
	                   }
	            },//success end
	       })//ajax end
            $nicknameMsg.show().removeClass("no").addClass("ok").text("좋은 닉네임 이네요!");
            return true;
        } else {
            $nicknameMsg.show().removeClass("ok").addClass('no').text("숫자, 영어, 한글로 2~6자로 입력해 주세요.");
            return false;
        }
    }//testNickname() end
    ...
    function testPassword() {
        ...
        if(passwordExp.test(password)) {
            $passwordMsg.show().removeClass("no").addClass("ok").text("좋은 비밀번호네요!");
            return true;
        } else {
            $passwordMsg.show().removeClass("ok").addClass('no').text("숫자, 영어 조합 6~12자로 입력해 주세요.");
            return false;
        }
    }//testPassword() end
    ...
    function testPasswordCheck() {
        ...
        if(password == passwordCheck) {
            $passwordCheckMsg.show().removeClass("no").addClass("ok").text("비밀번호가 일치합니다!");
            return true;
        } else {
            $passwordCheckMsg.show().removeClass("ok").addClass('no').text("비밀번호가 일치하지 않습니다.");
            return false;
        }
    }//testPasswordCheck() end
    ...
    function testName() {
        ...
        if(nameExp.test(name)) {
            $nameMsg.show().removeClass("no").addClass("ok").text("좋은 이름이네요!");
            return true;
        } else {
            $nameMsg.show().removeClass("ok").addClass('no').text("한글 2~6자로 입력해 주세요.");
            return false;
        }
    }//testName() end
    ...
    $passwordCheck.blur(testPasswordCheck);
    $nickname.blur(testNickname);
    $name.blur(testName);

    //가입하기를 눌렀을 때 유효성 검사가 통과돼야 넘어감
    $signUpForm.on("submit", function () {
        if(!testId()) return false;
        if(!testPassword()) return false;
        if(!testPasswordCheck()) return false;
        if(!testNickname()) return false;
    ...
    $allCheckbox.on("change", function () {
        if($(this).is(':checked')) {
            $('#popupBox input').prop('checked', true);
        } else {
            $('#popupBox input').prop('checked', false);
        }
    });//$allCheckbox.change end

    //팝업에서 동의를 눌렀을 때(유효성 검사)
    ...
    //팝업에서 모든 체크박스가 변화될 때(눌렀을 때)
    $('#popupBox input').on("change",function () {
        let checkboxNum = popupCheckboxCount();
        //체크박스가 2개미만이면 모두동의 체크를 해제 함
        if(checkboxNum < 2) {
    ...
    //체크박스가 몇 개 체크 돼있는지 카운트하는 함수
    function popupCheckboxCount() {
        let count = 0;
        if($requiredCheckbox.is(":checked")) {
            count++;
        }
        if($choiceCheckbox.is(":checked")) {
            count++;
        }
        return count;
    }//popupCheckboxCount() end
